...
def load_settings():
    if os.path.exists(SETTINGS_FILE):
        with open(SETTINGS_FILE, 'r') as f:
            ...
            except json.JSONDecodeError: return {"telegram_token": "", "telegram_id": ""}
...
def sync_with_firestore_api():
    ...
    if not user_id: return jsonify({"error": "Telegram User ID is not set in Settings."}), 400
    try:
...
def get_notes():
    ...
    for note in local_notes:
        notes_list.append({
            "id": note["id"], "mata_kuliah": note["mata_kuliah"],
            "deskripsi_tugas": note["deskripsi_tugas"], "deadline_timestamp": note["deadline_timestamp"],
            "tanggal_deadline_str": note["tanggal_deadline_str"], "deadline_iso_str": note["deadline_iso_str"],
            "status": note["status"], "user_id": note["user_id"] 
        })
...
def update_note_status_api(note_id):
    ...
    if not new_status: return jsonify({"error": "New status is not given"}), 400
